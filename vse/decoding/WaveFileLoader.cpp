...
namespace vse
{
    std::shared_ptr<ISeekableWaveSource> ReadOutToMemory(std::shared_ptr<IWaveSource> input)
    {
        if (!input) return nullptr;

        class XtlMemStreamWaveSourceImpl : public ISeekableWaveSource
        {
            PcmWaveFormat format_{};
            xtl::memory_stream ms_{};
        public:
            XtlMemStreamWaveSourceImpl(const PcmWaveFormat& format) : format_(format) {}
            [[nodiscard]] PcmWaveFormat GetFormat() const override { return format_; }
            [[nodiscard]] size_t Read(void* buffer, size_t buffer_length) override { return ms_.read(buffer, buffer_length); }
            [[nodiscard]] size_t GetTotalSampleCount() const override { return ms_.size() / format_.BlockAlign(); }
            [[nodiscard]] size_t GetSampleCursor() const override { return ms_.tellg() / format_.BlockAlign(); }
            size_t SetSampleCursor(size_t new_position) override { return ms_.seekg(new_position * format_.BlockAlign()) / format_.BlockAlign(); }
            size_t Write(const void* buffer, size_t buffer_length) { return ms_.write(buffer, buffer_length); }
        };

        auto destination = std::make_shared<XtlMemStreamWaveSourceImpl>(input->GetFormat());
        auto temp_buffer = xtl::temp_memory_buffer();
        while (true)
        {
